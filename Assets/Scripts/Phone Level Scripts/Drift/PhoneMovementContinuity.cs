...
public class PhoneMovementContinuity : MonoBehaviour
{
    ...
    private void OnTriggerEnter(Collider other)
    {
        if (other.gameObject.CompareTag("charger"))
        {
            ...
            ContinuityManager.instance.currentPhoneScript = this;
            ContinuityManager.instance.CheckLevelComplete();
            GetComponent<Rotation>().enabled = false;
            GetComponent<Collider>().isTrigger = true;
            GetComponent<PhoneMovementContinuity>().enabled = false;
        }
        ...
        {
            ShooterWithContinuity.instance.clipRangeVal = 0;
